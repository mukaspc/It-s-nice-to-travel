...
export class AIPlanGenerationService {
  ...
  constructor(openRouterService?: OpenRouterService, googlePlacesService?: GooglePlacesService) {
  ...
  async initializeGeneration(
    ...
    this.startGenerationProcess(plan, generationRecord.id, context).catch((error) => {
      ...
      for (const day of place.days) {
        ...
        for (const item of day.schedule) {
          if (item.activity !== "Travel" && item.activity !== "Check-in") {
            item.image_url = await this.googlePlaces.getPlacePhoto(item.activity, `${place.name}, ${location}`);
          }
        }

        // Enrich dining recommendations
        for (const item of day.dining_recommendations) {
          item.image_url = await this.googlePlaces.getPlacePhoto(item.name, `${place.name}, ${location}`);
        }
      }
    }
  ...
  "places": [
    {
      ...
      await this.updateGenerationStatus(generationId, "failed", 0, context).catch((updateError) => {
